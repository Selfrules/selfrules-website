...
export default async function CasaHunterPage({
  ...
  const proseClasses = `max-w-none leading-[1.7]
    prose prose-invert
    ...
    <>
      ...
      <Section>
        <div className="not-prose">
          ...
          >
            <source src="/videos/casahunter.mp4" type="video/mp4" />
          </video>
        </div>
      </Section>

      {/* Body — single continuous prose section, like other case studies */}
      <Section>
        <div className={proseClasses}>
          ...
          {/* V1 Dashboard */}
          <h2>{t('casahunter.v1.heading')}</h2>
          {v1Paragraphs.map((p, i) => (
            <p key={i} dangerouslySetInnerHTML={{ __html: markdownBold(p) }} />
          ))}
        </div>

        {/* Screenshots — break out of prose, stay in same Section */}
        ...
        <div className="not-prose mt-4 border border-[rgba(255,255,255,0.08)] overflow-hidden">
          <Image
        ...
        {/* Continue prose — turning point, approach, saas, learned */}
        ...
        <div className="not-prose mt-8 border border-[rgba(255,255,255,0.08)] p-6 md:p-8">
        ...
        {/* Resources */}
        <div className={`${proseClasses} mt-12`}>
          <h2>{t('casahunter.links.heading')}</h2>
